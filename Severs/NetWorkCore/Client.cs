...
namespace NetWork
{
    public class Client : NetCore
    {
        /// <summary>
        /// 使用的服务IP地址
        /// </summary>
        IPAddress seversAddress;
        /// <summary>
        /// 服务器端口
        /// </summary>
        ...
        public Client(IPAddress localAddress, int port = 80, int bufferSize = 1024)
        {
            this.seversAddress = localAddress;
            this.port = port;
            this.bufferSize = bufferSize;
        ...
        public bool Send(string msg, bool ShowMsg)
        {
            ...
            if (Encoding.Unicode.GetByteCount(msg) > 1024)
            {
                ...
                if(ShowMsg)
                    Notice(GetNoticeEventArgs.GetSendToSevers(this, msg));
                return true;
            }
            try
            {
            ...
            try
            {
                ...
                if (bytesRead > 0)//如果接受到数据
                {
                    ...
                    //普通模式
                    {
                        ...
                        {
                            Notice(GetNoticeEventArgs.GetReceiveFromSevers(this, msg));
                        }
                    }
                    else//大数据模式
                    {
                        ...
                        if (tempStringArray.Count() >= 2 && tempStringArray[tempStringArray.Count() - 2].EndsWith("【TCP大数据包传输完毕LongDataTransportEnding") == true)
                        {
                            ...
                            {
                                //匹配
                                Notice(GetNoticeEventArgs.GetReceiveFromSevers(this, longMsg));
                            }
                            else
                            {
                                //不匹配
                ...
                //    else
                //    {
                //        Notice(GetNoticeEventArgs.GetReceiveFromSevers(this, tempMessage));
                //        tempMessage = "";
        ...
        public IPEndPoint ipEndPoint
        {
            get
            {
                return new IPEndPoint(seversAddress, port);
            }
        }
    }
}
