...
namespace NetWork
{
    public class Severs : NetCore
    {
        ...
        public Severs(string localAddress, int port=80, int bufferSize=1024)
            : this(NetCore.GetIPAddress(localAddress)[0], port, bufferSize)
        {
        }
        public Severs(IPAddress localAddress, int port=80, int bufferSize = 1024)
            :this(new IPEndPoint(localAddress,port), bufferSize)
        {
        }
        public Severs(IPEndPoint ipEndPoint, int bufferSize = 1024)
        {
        ...
        public override bool Close()
        {
            ...
            myListener.Stop();
            isClosed = true;
            Notice(GetNoticeEventArgs.GetCloseSevers(this));
            return true;
        }
        ...
        public class User : NetCore
        {
            ...
            /// <param name="showSeversMsg">是否通知总监控，该选项对失败通知无效</param>
            /// <param name="showUserMsg">是否通知User监控，该选项对失败通知无效</param>
            /// <returns></returns>
            public bool Send(string msg, bool showSeversMsg, bool showUserMsg = true) 
            {
