...
class MainPhaseSelectUnitActionState : BattleStateBase
{
	...
	{
		get { return BattleGlobal.Core.battleInfo.unitSelected; }
	}
	...
	private void showUnitActionView()
	{
		...
		Chessboard.addChildOnLayer(this._unitActionView, BattleConsts.BattleFieldLayer_UI, cell.location.row, cell.location.col);
		// 禁用不可用的按钮
		this._btnAttack.GetComponent<Button>().interactable = cell.UnitOnCell.CanAttack;
		this._btnMove.GetComponent<Button>().interactable = cell.UnitOnCell.CanMove;
	...
	private void removeUnitActionView()
	{
		...
		UIEventListener.Get(this._btnAttack).onClick -= this.btnAttackClickHander;
		UIEventListener.Get(this._btnSkill).onClick -= this.btnSkillClickHander;
		GameObject.Destroy(this._unitActionView);
		this._isPopUp = false;
	}
